      ...
      <div class="flex items-center justify-between w-full h-full">
        ...
        <div class="flex flex-col items-center justify-center max-w-md w-full">
          ...
          <div class="flex items-center w-full">
            ...
            >
              <div
                class="absolute top-1/2 -translate-y-1/2 w-full h-1 bg-white/10 rounded-full overflow-hidden"
              >
                <!-- Barra de progreso actual -->
                <div
                  class="absolute top-0 left-0 h-full bg-white group-hover:bg-green-500 transition-all duration-150 ease-out"
                  :style="{
                    width: `${isDraggingProgress ? tempProgressPercentage : progressPercentage}%`,
                  }"
                ></div>
                <!-- Barra de hover -->
                <div
                  v-if="isHovering"
                  class="absolute top-0 left-0 h-full bg-white/20 transition-all duration-150"
                  :style="{ width: `${hoverProgressPercentage}%` }"
                ></div>
              </div>
              <!-- Punto de control -->
              <div
                class="absolute top-1/2 -translate-y-1/2 h-3 w-3 bg-white rounded-full -ml-1.5 transition-all duration-150 ease-out shadow-lg"
                :class="{
                  'opacity-0 group-hover:opacity-100': !isDraggingProgress,
                  'opacity-100 scale-125': isDraggingProgress,
                }"
                :style="{
                  left: `${isDraggingProgress ? tempProgressPercentage : progressPercentage}%`,
                }"
              ></div>
            </div>
...
// Define el nombre del componente como MusicPlayer
defineOptions({
  name: 'MusicPlayer',
})
...
// Estado adicional para la barra de progreso
...
onMounted(async () => {
  ...
  // Esperar a que el reproductor esté listo y sincronizado
  const waitForReady = async () => {
    ...
    // Continuar incluso si no está listo después de los intentos máximos
...
/* Animaciones suaves para la barra de progreso */
.transition-all {
...
/* Animación del punto de control */
.scale-125 {
  transform: scale(1.25);
}

/* Sombra para el punto de control */
.shadow-lg {
  box-shadow:
